...
func (s *OpenAIGatewayService) ForwardAsChatCompletions(
	...
	promptCacheBodyModified := false
	if account.Type == AccountTypeAPIKey {
		if trimmedKey := strings.TrimSpace(promptCacheKey); trimmedKey != "" {
			if existing, ok := promptCacheReqBody["prompt_cache_key"].(string); !ok || strings.TrimSpace(existing) == "" {
				promptCacheReqBody["prompt_cache_key"] = trimmedKey
