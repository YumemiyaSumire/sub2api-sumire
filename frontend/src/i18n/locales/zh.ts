export default {
  ...
  admin: {
    ...
    oauthSleeper: {
      title: 'OAuth 休眠保护',
      description: '根据 Sub2API 已记录的 OAuth 账号用量，自动把接近重置窗口的账号置为休眠限流',
      loadFailed: '加载 OAuth 休眠保护失败',
      saveFailed: '保存 OAuth 休眠设置失败',
      eventsFailed: '加载休眠事件失败',
      scanFailed: '手动扫描失败',
      saved: 'OAuth 休眠设置已保存',
      scanSuccess: '扫描完成，触发休眠 {triggered} 个账号',
      settingsTitle: '休眠设置',
      settingsHint: '默认关闭；启用后只扫描 active OAuth 账号，并通过账号仓储写入限流时间。',
      enabled: '启用 OAuth 休眠',
      enabledHint: '后台定时扫描接近阈值的 OpenAI / Anthropic OAuth 账号。',
      threshold: '触发阈值 (%)',
      thresholdHint: '范围 1 - 100；OpenAI 使用百分比，Anthropic fraction 会换算成百分比。',
      scanInterval: '扫描间隔 (秒)',
      intervalHint: '范围 30 - 86400 秒。',
      maxSleepPerScan: '单次扫描每分组最多新增',
      maxSleepHint: '范围 1 - 100；限制每次扫描中新休眠的账号数，每个选中分组分别计算。',
      platformScope: '扫描平台',
      groupScopeHint: '启用后只扫描这里选中的 OpenAI / Anthropic 分组；不选择分组不会扫描整个平台。',
      defaultThreshold: '默认触发阈值 (%)',
      groupThresholds: '分组阈值覆盖',
      groupThresholdsHint: '分组留空时使用默认阈值；账号同时属于多个选中分组时使用最低阈值。',
      saveSettings: '保存设置',
      scanOnce: '手动扫描一次',
      scanning: '扫描中...',
      running: '运行中',
      paused: '已暂停',
      lastError: '上次扫描错误：{error}',
      platformRequired: '至少需要启用一个扫描平台',
      groupRequired: '启用 OAuth 休眠前，请至少选择一个分组',
      noPlatformEnabled: '未启用平台',
      noGroupSelected: '未选择分组',
      groupScopeMeta: '{names} 等 {count} 个分组',
      intervalMeta: '每 {seconds} 秒扫描',
      effectiveIntervalMeta: '配置 {configured} 秒，实际 {effective} 秒',
      effectiveIntervalAcceleratedMeta: '配置 {configured} 秒，已加速到 {effective} 秒',
      accelerated: '已加速',
      acceleratedNoticeTitle: '智能加速生效中',
      acceleratedUntilMeta: '{names} 正在加速扫描，持续到 {time}',
      acceleratedGroupScopeMeta: '{names} 等 {count} 个分组',
      lastScanMeta: '扫描 {scanned} 个，触发 {triggered} 个',
      maxPerGroupMeta: '单次扫描每分组最多新增 {count} 个',
      sleepingAccounts: '当前休眠账号',
      sleepingAccountsHint: '仅展示 rate_limit_reset_at 仍在未来的 OpenAI / Anthropic OAuth 账号。',
      sleepingCount: '{count} 个休眠中',
      noSleepingAccounts: '暂无休眠中的 OAuth 账号',
      eventsTitle: '休眠事件',
      eventsHint: '事件只会在账号限流时间成功写入后记录，便于审计和回溯。',
      noEvents: '暂无休眠事件',
      unnamedAccount: '未命名账号',
      scanResult: '最近手动扫描：扫描 {scanned} 个账号，触发 {triggered} 个休眠事件。',
      thresholdMeta: '阈值 {threshold}',
      ...
      windows: {
        codex_5h: 'Codex 5 小时',
        codex_7d: 'Codex 7 天',
        session_window: 'Session 窗口',
        passive_usage_7d: '7 天被动用量',
      },
    },
